...
void compute_segment_precisions_double_1D_EE(double *oriData, int dataLength, double* pwrErrBound, unsigned char* pwrErrBoundBytes, double* err_est)
{
	int i = 0, j = 0, k = 0;
	double realPrecision = oriData[0]!=0?fabs(pw_relBoundRatio*oriData[0]):pw_relBoundRatio; 
	double approxPrecision;
	...
	{
		curValue = oriData[i];
		if (err_est[i] > cprss_tol) {
            cprss_tol = err_est[i];
        }
        if(i%segment_size==0&&i>0)
		{
			//get two first bytes of the realPrecision
	        realPrecision = err_est[i];
	        //realPrecision = cprss_tol;
            cprss_tol = 0;
            doubleToBytes(realPrecBytes, realPrecision);
			memset(&realPrecBytes[2], 0, 6);
			approxPrecision = bytesToDouble(realPrecBytes);
            ...
            //pwrErrBound[j] = err_est[j]; j++; //approxPrecision;
	        printf("real = %g, \t approx = %g, \t EE = %g\n", realPrecision, approxPrecision, err_est[i]);
	        pwrErrBound[j++] = approxPrecision;
            //pwrErrBound[j++] = err_est[i] >approxPrecision ? err_est[i] : approxPrecision;
	...
	printf("real = %g, \t approx = %g, \t EE = %g\n", realPrecision, approxPrecision, err_est[i-1]);
    //pwrErrBound[j] = err_est[j]; j++; //approxPrecision;
	//printf("approx = %g \t EE = %g\n", approxPrecision, err_est[i]);
    //pwrErrBound[j++] = err_est[i] >approxPrecision ? err_est[i] : approxPrecision;
	pwrErrBound[j++] = approxPrecision;
...
void compute_segment_precisions_double_2D_EE(double *oriData, double* pwrErrBound, 
int r1, int r2, int R2, int edgeSize, unsigned char* pwrErrBoundBytes, double Min, double Max, double* err_est)
{
	int i = 0, j = 0, k = 0, p = 0, index = 0, J; //I=-1,J=-1 if they are needed
	double realPrecision; 
	double approxPrecision;
...
void compute_segment_precisions_double_3D_EE(double *oriData, double* pwrErrBound, 
int r1, int r2, int r3, int R2, int R3, int edgeSize, unsigned char* pwrErrBoundBytes, double Min, double Max, double* err_est)
{
	int i = 0, j = 0, k = 0, p = 0, q = 0, index = 0, J = 0, K = 0; //I=-1,J=-1 if they are needed
	int r23 = r2*r3, ir, jr;
	double realPrecision; 
...
void SZ_compress_args_double_NoCkRngeNoGzip_1D_pwr_EE(unsigned char** newByteData, double *oriData, 
int dataLength, int *outSize, double min, double max, double* err_est)
{
	SZ_Reset();	
	int pwrLength = dataLength%segment_size==0?dataLength/segment_size:dataLength/segment_size+1;
	double* pwrErrBound = (double*)malloc(sizeof(double)*pwrLength);
	...
	compute_segment_precisions_double_1D_EE(oriData, dataLength, pwrErrBound, pwrErrBoundBytes, err_est);
...
void SZ_compress_args_double_NoCkRngeNoGzip_2D_pwr_EE(unsigned char** newByteData, double *oriData, int r1, int r2,
int *outSize, double min, double max, double* err_est)
{
	...
	compute_segment_precisions_double_2D_EE(oriData, pwrErrBound, r1, r2, R2, blockEdgeSize, pwrErrBoundBytes, min, max, err_est);
	unsigned int quantization_intervals;
	if(optQuantMode==1)
	{
		quantization_intervals = optimize_intervals_double_2D_pwr(oriData, r1, r2, R2, blockEdgeSize, pwrErrBound);
...
void SZ_compress_args_double_NoCkRngeNoGzip_3D_pwr_EE(unsigned char** newByteData, double *oriData, int r1, int r2, int r3, int *outSize, double min, double max, double* err_est)
{
	SZ_Reset();	
	int dataLength=r1*r2*r3;
	...
	compute_segment_precisions_double_3D_EE(oriData, pwrErrBound, r1, r2, r3, R2, R3, blockEdgeSize, pwrErrBoundBytes, min, max, err_est);	
	unsigned int quantization_intervals;
	if(optQuantMode==1)
	{
		quantization_intervals = optimize_intervals_double_3D_pwr(oriData, r1, r2, r3, R2, R3, blockEdgeSize, pwrErrBound);
